...
Pipeline xử lý:
    ...
    8. Bổ sung quận/huyện từ mô tả, bỏ tin không xác định được quận
    9. Xuất data/interim/df_raw.csv (tập làm sạch lần 1)

Chạy từ thư mục gốc dự án:
    python -m src.data.lam_sach_du_lieu
"""
...
def loai_trung_cheo_nguon(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.sort_values("_uu_tien", kind="stable").reset_index(drop=True)
...
def chay_pipeline() -> pd.DataFrame:
    """Chạy bước 1–8: đọc, hợp nhất, lọc, khử trùng. Không ghi file."""
    cfg = doc_config()
    goc = duong_dan_goc()
    ...
    nguon_su_dung = cfg.get("nguon_su_dung", list(bo_doc))
    for ten_nguon, ham_doc in bo_doc.items():
        if ten_nguon not in nguon_su_dung:
            print(f"  ⏭  Bỏ qua [{ten_nguon}]: không nằm trong cfg['nguon_su_dung']")
            continue
...
# Thứ tự quan trọng: tìm "Quận <số>" trước, rồi mới đến tên quận.
DANH_SACH_QUAN_THEO_TEN = [
...
def trich_quan_huyen(van_ban) -> str | None:
    """Trích tên quận/huyện TP.HCM từ một đoạn mô tả (None nếu không thấy)."""
    if pd.isna(van_ban):
        return None
    chu = re.sub(r"\s+", " ", str(van_ban)).strip().lower()

    # "quận 7", "Q.7", "q7"
    khop = re.search(r"\bquận\s*(1[0-2]|[1-9])\b", chu) or \
        re.search(r"\bq\.?\s*(1[0-2]|[1-9])\b", chu)
    if khop:
        return f"Quận {khop.group(1)}"

    for tu_khoa, ten_chuan in DANH_SACH_QUAN_THEO_TEN:
        if tu_khoa in chu:
            return ten_chuan
    return None


def bo_sung_quan_huyen(df: pd.DataFrame) -> pd.DataFrame:
    """
    Điền cột quan_huyen từ mo_ta_dac_diem cho các dòng đang trống.

    Cả CafeLand và batdongsan đều không có cột quận riêng, nên đây là
    cách duy nhất để có biến vị trí. Tin không trích được quận sẽ bị
    loại ở tao_df_raw() vì không phục vụ được câu hỏi phụ (Q2).
    """
    df = df.copy()
    df["quan_huyen"] = df["quan_huyen"].astype("object")
    thieu = df["quan_huyen"].isna() | df["quan_huyen"].astype(str).str.strip().eq("")
    df.loc[thieu, "quan_huyen"] = df.loc[thieu, "mo_ta_dac_diem"].apply(trich_quan_huyen)
    return df


def tao_df_raw(luu: bool = True, ten_file: str = "df_raw_chay_lai.csv") -> pd.DataFrame:
    """
    Tạo lại tập làm sạch lần 1 từ dữ liệu thô.

    Gồm bước 1–8 của chay_pipeline() + bổ sung quận + bỏ tin không rõ quận.

    Mặc định ghi ra df_raw_chay_lai.csv, KHÔNG ghi đè df_raw.csv:
    df_raw.csv là bản nhóm đã chốt (dùng cho Project Charter và Data
    Dictionary). Lần chạy gốc sắp xếp bằng quicksort (không ổn định) khi
    khử trùng chéo nguồn, nên chạy lại cho cùng 11.888 dòng và cùng phân bố
    nguồn nhưng một số ít tin trùng được giữ bản đại diện khác.
    """
    cfg = doc_config()
    df = chay_pipeline()

    in_tieu_de("BƯỚC 9: BỔ SUNG QUẬN/HUYỆN TỪ MÔ TẢ")
    df = bo_sung_quan_huyen(df)
    truoc = len(df)
    df = df.dropna(subset=["quan_huyen"]).reset_index(drop=True)
    print(f"  Bỏ tin không xác định được quận: {truoc:,} -> {len(df):,} "
          f"(loại {truoc - len(df):,})")

    if luu:
        file_ra = duong_dan_goc() / cfg["duong_dan"]["du_lieu_trung_gian"] / ten_file
        file_ra.parent.mkdir(parents=True, exist_ok=True)
        df.to_csv(file_ra, index=False)
        print(f"\n✅ Đã lưu: {file_ra}  ({len(df):,} dòng × {df.shape[1]} cột)")
    return df
