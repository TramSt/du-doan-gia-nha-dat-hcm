...
SCHEMA_CHUNG = [
    ...
    "gia_ban_trieu",     # Giá bán (triệu VNĐ)
...
def doc_cafeland(duong_dan: Path) -> pd.DataFrame:
    ...
    ket_qua = pd.DataFrame({
        ...
        "gia_ban_trieu": df["Gia_ban"].apply(phan_tich_gia_text),
        ...
        "phuong_xa": np.nan,       # nguồn này không có
...
def doc_batdongsan(duong_dan: Path) -> pd.DataFrame:
    ...
    """
    if str(duong_dan).endswith(".xlsb"):
        df = pd.read_excel(duong_dan, engine="pyxlsb")
    else:
        df = pd.read_csv(duong_dan)
...
def phan_loai_giao_dich(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    gia_qua_thap = df["gia_ban_trieu"] < nguong_gia_ban_toi_thieu
...
def chuan_hoa_loai_hinh(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    df["loai_hinh"] = (
        ...
        .fillna("Khong ro")             # nhãn cho dòng bị thiếu
    )
...
def loc_ngoai_lai(df: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    ...
    df = df[
        df["gia_ban_trieu"].between(ng_gia["min"], ng_gia["max"])
    ...
    if cfg["nguong_loc"]["dung_iqr"]:
        ...
        for cot in ["gia_ban_trieu", "dien_tich_m2"]:
...
def loai_trung_cheo_nguon(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["_khoa_gia"] = (df["gia_ban_trieu"] / 100).round(0) * 100
...
def chay_pipeline() -> pd.DataFrame:
    ...
    truoc = len(df)
    df = df.dropna(subset=["gia_ban_trieu", "dien_tich_m2"])
    ...
    df["gia_moi_m2_trieu"] = df["gia_ban_trieu"] / df["dien_tich_m2"]
